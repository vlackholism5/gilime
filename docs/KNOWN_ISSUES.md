...
- **route_label 컬럼:** app_alert_events.route_label은 v1.4-07 DDL(sql/releases/v1.4/schema/v1.4-07_route_label.sql) 적용 후 필터·Review 링크에 사용. 미적용 시 alerts route/subscribed 필터는 비활성.
- **generate_from_metrics 0건:** 직전 PARSE_MATCH 대비 NONE/LOW 증가가 없으면 inserted 0은 정상(데이터 조건 미충족).
