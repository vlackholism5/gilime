...
- sql/releases/v1.4/schema/v1.4-07_route_label.sql 실행 후, `php scripts/php/run_alert_generate_from_metrics.php` (또는 Windows: `C:\xampp\php\php.exe scripts\php\run_alert_generate_from_metrics.php`) 실행.
...
## E2E 검증: 이벤트 1건 생성 → alerts 노출 → deliveries 1건 기록

- **1)** sql/releases/v1.4/schema/v1.4-10_e2e_test_event.sql 1번 블록 실행 → app_alert_events에 E2E 테스트 이벤트 1건 INSERT (content_hash NOW 기반, E2E 전용).
- **2)** /user/alerts.php 접속(해당 이벤트가 목록에 노출되는지 확인).
- **3)** 동일 SQL 파일의 검증 SELECT 3개 실행 → app_alert_events 건수, app_alert_deliveries 건수, 최근 deliveries 20건 확인. alerts 노출 후 deliveries가 1건 이상 증가했으면 통과.
