...
- `php scripts/php/run_delivery_outbound_stub.php --limit=200`: status='pending' 행을 created_at ASC로 LIMIT, 각 row를 status='sent', delivered_at=NOW(), last_error=NULL로 업데이트. 예외 시 status='failed', last_error 저장.
