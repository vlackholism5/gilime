...
## Reset Baseline (Phase 0 — 2026-02-14)

- **목표:** MVP by 2026-02-20. SoT → DB → API → UI → Ops 순서로 clean reset baseline 확보.
- **원칙:** Product Eng + Data Eng + Platform/SRE + Security + QA. diff patches만, 동작 코드 삭제 금지. Reset = "make it controllable".
- **변경 규칙:** why / what / how to verify 필수. /docs/SOT를 Source of Truth로 유지 (현재는 RESET 문서만 생성).

### Phase 0: SNAPSHOT (코드 변경 없음)

1. **Git tag 생성 (실행하지 말고 명령만 제공):**
   ```bash
   git add -A
   git status   # 변경사항 확인
   git commit -m "chore: pre-reset snapshot 2026-02-14"   # 필요 시
   git tag pre_reset_2026-02-14
   git push origin pre_reset_2026-02-14   # 원격 반영 시
   ```

2. **docs/STATUS_FOR_GPT.md:** 이 섹션으로 갱신 완료.
...
- **v1.7-16** parse_status/batch policy 통일. `scripts/php/run_parse_match_batch.php` 선별 규칙 고정(only_failed=1: failed+legacy failed job, 기본: success), dry_run 3줄 요약 + selected_by_status/selected_reason_top 추가. docs/releases/v1.7/specs/spec_16_parse_status_policy.md, docs/releases/v1.7/smoke/smoke_16_parse_status_policy.md, docs/releases/v1.7/gate/gate_16_parse_status_policy.md.
- **v1.7-15** Legacy error normalization. `app/inc/lib/error_normalize.php` 추가, `public/admin/doc.php` Failure TopN/last_parse_error_code를 표준 코드 집계로 통일해 UNKNOWN 축소. docs/releases/v1.7/specs/spec_15_legacy_error_normalize.md, docs/releases/v1.7/smoke/smoke_15_legacy_error_normalize.md, docs/releases/v1.7/gate/gate_15_legacy_error_normalize.md.
...
- **v1.6** 핵심: admin alert_ops (list + create + contract), alert_event_audit (filters + summary + drilldown), user alerts delivery guard, docs/releases/v1.6/gate/v1.6_RELEASE_GATE.md S1–S7 + Evidence SQL 블록.
...
- **v1.5-03** MVP2.5 hardening: observability baseline, alert ref contract, delivery semantics + pagination.
  ...
  - **v1.5-02:** Alert ref contract. docs/ALERT_REF_CONTRACT_v1_5.md (ref_type route/doc/NULL, Review 링크 규칙). run_alert_ingest_stub ref_type=route·ref_id=1·route_label=R1. sql/releases/v1.5/validation/v1.5-02_validation.sql (read-only 위반 탐지).
...
- **v1.4-00 (planning-only)** MVP2 v1.4 계획 문서 4종. docs/references/PRD_v1_4_MVP2.md, ARCH_v1_4_SYSTEM.md, ERD_v1_4_DRAFT.md, WIREFRAME_v1_4.md.
...
- **검증:** sql/archive/v0.6/validation/v0.6-22_validation.sql (8개 쿼리: 테이블 존재, metrics row count, candidate 집계 일치, UPSERT idempotent).
...
- **검증:** sql/archive/v0.6/validation/v0.6-21_validation.sql (8개 쿼리: LOW pending/approved, alias canonical 존재, alias_text 길이, 회귀)
- **매칭 로직/SoT 불변:** 승인/등록 게이트만 강화.

## v0.6-20 요약

- **Import 스크립트:** scripts/php/import_seoul_bus_stop_master_full.php (euc-kr CSV → UTF-8, UPSERT, idempotent)
...
- **검증:** sql/archive/v0.6/validation/v0.6-18_validation.sql 에 검증 쿼리 7개 (주석 블록, Workbench 실행용). 매칭 로직/SoT 변경 없음.

## SQL 검증 (GPT 대화에서)

- **GPT는 SQL을 실행할 수 없음.** 사용자가 Workbench(또는 Cursor 터미널)에서 실행 후, **실행 결과를 GPT 대화창에 붙여넣으면** GPT가 결과를 해석하고 다음 지시(정상/추가 점검 등)를 낸다.
- 절차: (1) GPT에게 "검증 쿼리 알려줘" 요청 → (2) GPT가 sql/archive/v0.6/validation/v0.6-18_validation.sql 기준으로 쿼리+치환값 제시 → (3) 사용자 실행 → (4) 결과 붙여넣기 → (5) GPT가 다음 지시.
...
## Current state summary (RESET 2026-02-14)

- Reset baseline 작업 기준일: 2026-02-14. 목표: MVP by 2026-02-20.
- SoT → DB → API → UI → Ops 순서로 제어 가능한 상태(controllable) 확보 중.
- docs/RESET/00_SNAPSHOT.md, 01_INVENTORY.md 참고.
