 ...
 *
 * Usage:
 *   c:\xampp\php\php.exe scripts/run_parse_match_batch.php --limit=20
 *   c:\xampp\php\php.exe scripts/run_parse_match_batch.php --only_failed=1 --limit=50
 *   c:\xampp\php\php.exe scripts/run_parse_match_batch.php --source_doc_id=123
 *   c:\xampp\php\php.exe scripts/run_parse_match_batch.php --dry_run=1 --limit=10
 */

require_once __DIR__ . '/../app/inc/db.php';
require_once __DIR__ . '/../app/inc/pdf_parser.php';
require_once __DIR__ . '/../app/inc/observability.php';
...
function runParseForDoc(PDO $pdo, int $docId, int $requestedBy, string $traceId): array {
  ...
  $doc = $docStmt->fetch();
  if (!$doc) return ['ok' => false, 'error_code' => 'DOC_NOT_FOUND', 'rows' => 0, 'error' => 'source_doc not found'];

  $filePath = trim((string)$doc['file_path']);
  if ($filePath === '') return ['ok' => false, 'error_code' => 'FILE_PATH_EMPTY', 'rows' => 0, 'error' => 'file_path empty'];

  $projectRoot = (string)realpath(__DIR__ . '/..');
  $absolutePath = resolvePdfPathForBatch($projectRoot, $filePath);
  if ($absolutePath === null) return ['ok' => false, 'error_code' => 'INVALID_OR_MISSING_FILE', 'rows' => 0, 'error' => 'invalid/missing pdf file'];
  ...
  if (!$parseResult['success']) {
    ...
    $errCode = (string)($parseResult['error_code'] ?? 'PARSE_FAILED');
  ...
  if ($routeLabel === '' || $stops === []) {
    updateParseStatusBatch($pdo, $docId, 'failed');
    return ['ok' => false, 'error_code' => 'EMPTY_PARSE_RESULT', 'rows' => 0, 'error' => 'empty parse result'];
  }
  ...
  try {
    ...
    return ['ok' => false, 'error_code' => 'BATCH_RUN_EXCEPTION', 'rows' => 0, 'error' => mb_substr($e->getMessage(), 0, 220)];
  }
}
...
if ($sourceDocId > 0) {
  ...
  // v1.7-16 정책: only_failed=0 기본은 success 상태만 대상
  $sql = "
    SELECT id, file_path, parse_status, 'parse_status_success' AS selected_reason
    FROM shuttle_source_doc
...
$stats = [
  ...
  'selection_policy' => $sourceDocId > 0
    ...
    : ($onlyFailed
      ...
      : 'only_failed=0 => parse_status=success'),
  'dry_run' => $dryRun,
];
$failCounts = [];
$selectedByStatus = [];
